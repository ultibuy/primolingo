...
// ─── Design tokens (from primolingo-tokens design system) ───
const T = {
  // existing fields (same values, keep them)
  bg1: "#1e1e2e",
  bg2: "#2d2b55",
  bg3: "#1a1a2e",
  primary: "#a78bfa",
  ...
  fontKid: "'Fredoka', 'Plus Jakarta Sans', sans-serif",   // NEW
};

function LandingIcon({ name, size = 20, color = T.primary, secondary = T.primaryLight }) {
  const props = {
    width: size,
    height: size,
    viewBox: '0 0 40 40',
  ...
  if (name === 'diamond') {
    return (
      <svg {...props}>
        <path d="M12 8h16l6 8-14 17L6 16l6-8Z" fill={`${color}33`} stroke={color} strokeWidth="2.2" strokeLinejoin="round" />
  ...
  if (name === 'crown') {
    return (
      <svg {...props}>
        <path d="M8 29h24l2-15-8 6-6-10-6 10-8-6 2 15Z" fill={`${color}33`} stroke={color} strokeWidth="2.4" strokeLinejoin="round" />
  ...
  if (name === 'medal') {
    return (
      <svg {...props}>
        <path d="m14 5 6 9 6-9M20 14v6" stroke={secondary} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
        <circle cx="20" cy="25" r="9" fill={`${color}30`} stroke={color} strokeWidth="2.3" />
        <path d="M20 20.5v9M16.8 23.5 20 20.5l3.2 3" stroke={secondary} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }
  ...
  if (name === 'trophy') {
    return (
      <svg {...props}>
        <path d="M14 10h12v5c0 6-2.7 10-6 10s-6-4-6-10v-5Z" fill={`${color}33`} stroke={color} strokeWidth="2.4" />
        <path d="M14 13H9c0 4 2 7 5.7 7.8M26 13h5c0 4-2 7-5.7 7.8" stroke={color} strokeWidth="2.4" strokeLinecap="round" />
  ...
  if (name === 'spark') {
    return (
      <svg {...props}>
        <path d="M20 5l2.7 9.2L32 17l-9.3 2.8L20 29l-2.7-9.2L8 17l9.3-2.8L20 5Z" fill={`${color}44`} stroke={color} strokeWidth="2" strokeLinejoin="round" />
        <path d="M31 27l1.2 3.8L36 32l-3.8 1.2L31 37l-1.2-3.8L26 32l3.8-1.2L31 27Z" fill={secondary} />
      </svg>
    );
  }
  ...
  if (name === 'bronze') {
    return (
      <svg {...props}>
        <circle cx="20" cy="20" r="12" fill={`${color}30`} stroke={color} strokeWidth="2.4" />
        <path d="M15 20h7a4 4 0 1 1 0 8h-7V12h6a4 4 0 0 1 0 8" stroke={secondary} strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }
  ...
  return (
    <svg {...props}>
      <circle cx="20" cy="20" r="12" fill={`${color}28`} stroke={color} strokeWidth="2.4" />
      <path d="M20 11v18M12 20h16" stroke={secondary} strokeWidth="2.4" strokeLinecap="round" />
    </svg>
  );
}
...
const css = {
  page: {
    ...
    fontFamily: T.font,
    overflowX: "hidden",
    position: "relative",  // establishes containing block for absolute canvas
  },
...
// Absolutely positioned — scrolls with the page, stars distributed over full height
function StarField({ containerRef }) {
  ...
  useEffect(() => {
    ...
    const starColors = ["#ffffff", "#fbbf24", "#c4b5fd"];
    ...
    const makeStar = (x, y, w, h, opts = {}) => ({
      x: x !== undefined ? x : Math.random() * w,
      y: y !== undefined ? y : Math.random() * h,
      r: (opts.r || (1.0 + Math.random() * 1.8)),
      color: opts.color || starColors[Math.floor(Math.random() * starColors.length)],
      opacity: (opts.opacity || (0.7 + Math.random() * 0.25)),
      twinkleOffset: Math.random() * Math.PI * 2,
      twinkleSpeed: 0.3 + Math.random() * 1.0,
    });

    const buildStars = (w, h) => {
      const firstViewportH = Math.min(
        typeof window !== "undefined" ? window.innerHeight || 820 : 820,
        h
      );
      const heroH = Math.min(900, Math.max(firstViewportH, h * 0.15)); // first screen ~900px

      // Base layer: distributed across full page
      const baseCount = Math.max(160, Math.min(Math.round((w * h) / 9000), 320));
      const baseStars = Array.from({ length: baseCount }, () => makeStar(undefined, undefined, w, h));

      // Extra concentration on the first screen (hero) — visible immediately on landing.
      const heroStars = Array.from({ length: 55 }, () =>
        makeStar(undefined, Math.random() * firstViewportH, w, h, { r: 0.7 + Math.random() * 1.4, opacity: 0.58 + Math.random() * 0.22 })
      );
      const landingStars = [
        [0.08, 0.21, 1.2, "#ffffff"], [0.27, 0.18, 1.0, "#ffffff"],
        [0.46, 0.30, 1.1, "#c4b5fd"], [0.66, 0.43, 1.4, "#fbbf24"],
        [0.86, 0.38, 1.4, "#c4b5fd"], [0.18, 0.67, 1.0, "#ffffff"],
        [0.72, 0.70, 1.1, "#ffffff"],
      ].map(([xPct, yPct, r, color]) =>
        makeStar(w * xPct, firstViewportH * yPct, w, h, {
          r,
          color,
          opacity: 0.72,
        })
      ...
      stars = [...baseStars, ...heroStars, ...landingStars];

      // Sparkles: 1 per ~500px height, at least 4 in the first screen
      const totalSparkles = Math.max(8, Math.round(h / 500));
      const heroSparkles = Array.from({ length: 4 }, () => ({
        x: Math.random() * w,
        y: Math.random() * firstViewportH,
      ...
      const landingSparkles = [
        [0.32, 0.34, 5, "#c4b5fd"], [0.82, 0.58, 6, "#fbbf24"],
      ].map(([xPct, yPct, size, color]) => ({
        x: w * xPct,
        y: firstViewportH * yPct,
        size,
        color,
        opacity: 0.72,
        twinkleOffset: Math.random() * Math.PI * 2,
        twinkleSpeed: 0.55 + Math.random() * 0.7,
      }));
      const restSparkles = Array.from({ length: Math.max(0, totalSparkles - 4) }, () => ({
        x: Math.random() * w,
        y: heroH + Math.random() * (h - heroH),
      ...
      sparkles = [...heroSparkles, ...landingSparkles, ...restSparkles];
    };
    ...
    const syncSize = () => {
      ...
      const h = container.offsetHeight || document.documentElement.scrollHeight;
    ...
    function draw() {
      ...
      stars.forEach((s) => {
        ...
        ctx.save();
        ctx.globalAlpha = s.opacity * (0.45 + 0.55 * twinkle);
      ...
      sparkles.forEach((sp) => {
        const twinkle = 0.5 + 0.5 * Math.sin(elapsed * sp.twinkleSpeed + sp.twinkleOffset);
        drawSparkle(sp.x, sp.y, sp.size, sp.color, sp.opacity * (0.4 + 0.6 * twinkle));
      });

      animFrame = requestAnimationFrame(draw);
    }

    // Use ResizeObserver to detect when the page finishes rendering
  ...
  return (
    <div style={{ fontSize: 11 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, padding: "4px 0" }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, fontFamily: T.fontKid }}>Bonjour Damien 👋</div>
...
function MockQuiz() {
  ...
  return (
    <div style={{ fontSize: 11, textAlign: "center", padding: "8px 0" }}>
      ...
      <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 24, lineHeight: 1.4, fontFamily: T.fontKid }}>
        Il va <span style={{
...
function MockLevelPath() {
  const levels = [
    { name: "Bronze", icon: "bronze", color: "#cd7f32", reached: true },
    { name: "Argent", icon: "medal", color: "#c0c0c0", reached: true },
    { name: "Couronne", icon: "crown", color: T.gold, reached: true },
    { name: "Diamant", icon: "diamond", color: T.primary, reached: true },
  ];
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <div style={{
          ...
          <LandingIcon name="diamond" size={21} color={T.primary} />
        </div>
        <div>
        ...
        {levels.map((l, i) => (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", position: "relative" }}>
            ...
            <div style={{
              ...
              <LandingIcon name={l.icon} size={19} color={l.color} secondary={l.name === "Argent" ? "#f8fafc" : T.primaryLight} />
            </div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: T.green, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          <LandingIcon name="spark" size={16} color={T.green} secondary={T.primaryLight} /> Maîtrisée — diamant brillant
        </span>
      </div>
    </div>
  );
...
function RevisionTimeline() {
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 4, justifyContent: "center", marginTop: 32 }}>
      {stages.map((s, i) => (
        <div key={i} style={{ flex: 1, textAlign: "center", position: "relative" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, marginBottom: 8, minHeight: 60, justifyContent: "flex-end" }}>
            ...
            {s.dots === 0 && (
              <LandingIcon name="spark" size={22} color={s.color} secondary={T.primaryLight} />
            )}
          </div>
          <div style={{ height: 18, position: "relative", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 4 }}>
            {i < stages.length - 1 && (
              <div style={{
...
function HeroStars() {
  const stars = [
    { x: "7%", y: "18%", s: 3, c: "#ffffff", d: "0s" },
    { x: "23%", y: "24%", s: 2, c: "#ffffff", d: "1.1s" },
    { x: "45%", y: "34%", s: 2, c: "#ffffff", d: "1.6s" },
    { x: "52%", y: "13%", s: 2, c: "#c4b5fd", d: "0.5s" },
    { x: "60%", y: "55%", s: 3, c: "#ffffff", d: "1.3s" },
    { x: "69%", y: "28%", s: 2, c: "#fbbf24", d: "0.9s" },
    { x: "88%", y: "21%", s: 2, c: "#ffffff", d: "0.4s" },
    { x: "10%", y: "76%", s: 2, c: "#ffffff", d: "1.9s" },
    { x: "73%", y: "78%", s: 2, c: "#ffffff", d: "1.5s" },
  ];

  const sparkles = [
    { x: "55%", y: "25%", s: 16, c: "#fbbf24", d: "1.1s" },
    { x: "83%", y: "35%", s: 15, c: "#ffffff", d: "0.6s" },
  ];

  return (
    <div className="oq-hero-stars" aria-hidden="true">
      {stars.map((star, i) => (
        <span
          key={`star-${i}`}
          className="oq-hero-star-dot"
          style={{
            left: star.x,
            top: star.y,
            width: star.s,
...
// ═══════════════════════════════════════
// LANDING PAGE V5
// Based on V4 with design system tokens from primolingo-tokens integrated
// ═══════════════════════════════════════
export default function LandingPageV5() {
  ...
  const triggerRandomAnimation = () => {
    ...
    setCelebAnim(pick);
    posthog.capture('landing_demo_animation_clicked', { animation: pick });
  };
  ...
  return (
    <div style={css.page} ref={containerRef}>
        <style>{`
          html, body {
            margin: 0;
            background: #1e1e2e;
          ...
          @media (max-width: 768px) {
            ...
            .oq-final-title { font-size: 26px !important; line-height: 1.14 !important; }
          }
          ...
          .oq-hero-sparkle::before,
          .oq-hero-sparkle::after {
            content: "";
            position: absolute;
            left: 50%;
            top: 50%;
        ...
        {/* Star field — absolute, scrolls with page, distributed over full height */}
        <StarField containerRef={containerRef} />

        {/* All content above the star canvas */}
        ...
        {/* ─── NAV ─── */}
        <nav style={{ ...css.nav, position: "sticky", zIndex: 100 }} className="oq-nav" data-section="nav">
          <div style={css.navLogo}>
            <svg style={css.navLogoIcon} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
              <defs>
                <linearGradient id="nl5-bg" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0" stopColor="#1e1e2e"/>
                  <stop offset="0.5" stopColor="#2d2b55"/>
                  <stop offset="1" stopColor="#1a1a2e"/>
                </linearGradient>
                <linearGradient id="nl5-r" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0" stopColor="#c4b5fd"/>
                  <stop offset="1" stopColor="#a78bfa"/>
                </linearGradient>
                <linearGradient id="nl5-f" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0" stopColor="#fbbf24"/>
                  <stop offset="1" stopColor="#fb923c"/>
                </linearGradient>
              </defs>
              <rect width="100" height="100" rx="22.37" fill="url(#nl5-bg)"/>
              ...
              <g transform="translate(-0.23 -7.76) rotate(-20 50 50)">
                ...
                <path d="M50 12 Q70 30 70 65 L60 65 Q60 30 50 12 Z" fill="url(#nl5-r)" opacity="0.25"/>
                <circle cx="50" cy="40" r="9" fill="url(#nl5-r)"/>
              ...
              <g transform="translate(-4.65 -5.54) rotate(-20 50 50)">
                <path d="M40 68 Q44 88 48 68 Z" fill="url(#nl5-f)"/>
              </g>
              <g transform="translate(-4.92 -5.42) rotate(-20 50 50)">
                <path d="M48 68 Q51 84 55 68 Z" fill="url(#nl5-f)"/>
                <path d="M55 68 Q58 84 62 68 Z" fill="url(#nl5-f)"/>
              </g>
              <g transform="translate(-4.91 -5.4) rotate(-20 50 50)">
                <path d="M62 68 Q66 88 70 68 Z" fill="url(#nl5-f)"/>
              </g>
            </svg>
            PrimoLingo
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <button style={css.navCta}
              onClick={() => { posthog.capture('landing_cta_clicked', { location: 'nav' }); navigate('/login'); }}
        ...
        {/* ─── HERO ─── */}
        <section style={{ ...css.hero, position: "relative", zIndex: 1 }} className="oq-hero" data-section="hero">
          <HeroStars />
          <div style={{ ...css.heroText, position: "relative", zIndex: 1 }}>
          ...
          <div style={{ ...css.heroVisual, position: "relative", zIndex: 1 }}>
        ...
        <div style={{ ...css.sectionDivider, position: "relative", zIndex: 1 }} />

        {/* ─── LE PROBLÈME ─── */}
        <section style={{ ...css.section, position: "relative", zIndex: 1 }} className="oq-section" data-section="problem">
          ...
          <div style={css.problemGrid}>
            <div style={css.problemCard}>
              <div style={css.problemEmoji}>
                <svg width="42" height="42" viewBox="0 0 640 640" fill="none">
                  ...
                  <path d="M184 120C184 89.1 209.1 64 240 64L264 64C281.7 64 296 78.3 296 96L296 544C296 561.7 281.7 576 264 576L232 576C202.2 576 177.1 555.6 170 528C169.3 528 168.7 528 168 528C123.8 528 88 492.2 88 448C88 430 94 413.4 104 400C84.6 385.4 72 362.2 72 336C72 305.1 89.6 278.2 115.2 264.9C108.1 252.9 104 238.9 104 224C104 179.8 139.8 144 184 144L184 120zM456 120L456 144C500.2 144 536 179.8 536 224C536 239 531.9 253 524.8 264.9C550.5 278.2 568 305 568 336C568 362.2 555.4 385.4 536 400C546 413.4 552 430 552 448C552 492.2 516.2 528 472 528C471.3 528 470.7 528 470 528C462.9 555.6 437.8 576 408 576L376 576C358.3 576 344 561.7 344 544L344 96C344 78.3 358.3 64 376 64L400 64C430.9 64 456 89.1 456 120z" stroke="#a78bfa" strokeWidth="35" fill="none"/>
                  <g clipPath="url(#brainClip)">
                    <rect x="0" y="0" width="640" height="640" fill="#a78bfa" opacity="0.70" style={{ animation: "brainDrain 2s ease-in-out infinite" }}/>
                  </g>
                </svg>
              </div>
            ...
            <div style={css.problemCard}>
              <div style={css.problemEmoji}>
                ...
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none">
                  <circle cx="12" cy="12" r="10" stroke="#a78bfa" strokeWidth="1.8" style={{ animation: "ring1 3s steps(1) infinite" }}/>
                  <circle cx="12" cy="12" r="6.5" stroke="#c4b5fd" strokeWidth="1.5" style={{ animation: "ring2 3s steps(1) infinite" }}/>
                  <circle cx="12" cy="12" r="3" stroke="#a78bfa" strokeWidth="1.5" style={{ animation: "ring3 3s steps(1) infinite" }}/>
                  <circle cx="12" cy="12" r="1.2" fill="#a78bfa" style={{ animation: "ring4 3s steps(1) infinite" }}/>
                </svg>
              </div>
        ...
        {/* ─── LA SOLUTION ─── */}
        <div style={{ position: "relative", zIndex: 1 }} data-section="solution">
          <div style={{
            position: "absolute", inset: 0,
            background: "radial-gradient(ellipse 80% 40% at 50% 50%, rgba(124,58,237,0.05) 0%, transparent 100%)",
            pointerEvents: "none",
          }} />
        ...
        <section style={{ ...css.section, position: "relative", zIndex: 1 }} className="oq-section" data-section="game">
        ...
        <section style={{ ...css.section, position: "relative", zIndex: 1 }} className="oq-section" data-section="smart">
        ...
        {/* ─── ESPACE PARENT ─── */}
        <section style={{ ...css.section, position: "relative", zIndex: 1 }} className="oq-section" data-section="parent">
        ...
        {/* ─── RULES COVERED ─── */}
        <section style={{ ...css.section, position: "relative", zIndex: 1 }} className="oq-section" data-section="program">
        ...
        {/* ─── CHIFFRES ─── */}
        <section style={{ ...css.section, position: "relative", zIndex: 1 }} className="oq-section" data-section="stats">
        ...
        {/* ─── GRATUIT ─── */}
        <section style={{ ...css.section, position: "relative", zIndex: 1 }} className="oq-section" data-section="free">
        ...
        {/* ─── FINAL CTA ─── */}
        <section style={{ ...css.finalCta, position: "relative", zIndex: 1 }} data-section="cta">
          <h2 className="oq-final-title" style={{ fontFamily: T.fontDisplay, fontSize: 32, fontWeight: 800, marginBottom: 16, textWrap: "balance" }}>
        ...
        {/* ─── FOOTER ─── */}
        <footer style={{ ...css.footer, position: "relative", zIndex: 1 }} data-section="footer">
          <p>Fait avec ❤ pour les enfants qui veulent dompter l'orthographe</p>
          <p style={{ marginTop: 8, display: 'flex', justifyContent: 'center', gap: '1.2rem', flexWrap: 'wrap' }}>
            <a href="/legal" style={{ color: T.textSubtle, textDecoration: 'none', borderBottom: `1px solid ${T.glassBorder}` }}>
              Mentions légales & Confidentialité
            </a>
            <a href="/regles" style={{ color: T.textSubtle, textDecoration: 'none', borderBottom: `1px solid ${T.glassBorder}` }}>
