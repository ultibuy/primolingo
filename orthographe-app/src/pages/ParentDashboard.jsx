...
function ChildEditForm({ uid, child, onSaved, onCancel, onDeleted }) {
  ...
  const hasQuiz = child.progress != null;
  ...
  async function handleDelete() {
    setDeleting(true);
    try {
      await deleteChild(uid, child.id);
      onDeleted?.(child.id);
    } catch (e) {
      captureException(e);
  ...
  return (
    <div style={{ display: 'grid', gap: '0.75rem' }}>
      ...
      <div style={{ marginTop: '0.5rem', paddingTop: '0.9rem', borderTop: '1px solid rgba(248,113,113,0.15)' }}>
        <div style={{ fontSize: '0.72rem', fontWeight: 800, color: 'rgba(248,113,113,0.6)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.6rem' }}>
          Suppression du profil
        </div>
        <button
          type="button"
          onClick={() => hasQuiz ? setShowDeleteConfirm(true) : handleDelete()}
          disabled={deleting}
          style={{ ...dangerBtnStyle, opacity: deleting ? 0.5 : 1 }}
        >
      ...
      {showDeleteConfirm && (
        <PopupModal
          ...
          showClose={!deleting}
          panelStyle={{ textAlign: 'center', padding: '1.8rem 1.6rem' }}
        >
          <div style={{ fontSize: '2rem', marginBottom: '0.75rem' }}>🗑️</div>
          <h3 style={{ margin: '0 0 0.5rem', fontSize: '1.05rem', fontWeight: 800, color: '#fff' }}>
            Supprimer le profil de {child.name} ?
          </h3>
          {hasQuiz && (
            <p style={{ margin: '0 0 1.2rem', fontSize: '0.83rem', color: '#fca5a5', lineHeight: 1.6 }}>
              Cette action supprimera définitivement toute sa progression enregistrée — règles apprises, pièces, historique et sauvegardes. Elle est <strong>irréversible</strong>.
            </p>
          )}
          {!hasQuiz && (
            <p style={{ margin: '0 0 1.2rem', fontSize: '0.83rem', color: '#9ca3af', lineHeight: 1.6 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <button
              ...
              disabled={deleting}
              style={{ ...dangerBtnStyle, width: '100%', padding: '10px 0', opacity: deleting ? 0.5 : 1, background: 'rgba(248,113,113,0.15)' }}
            >
...
function GestionEnfantsSection({ children, uid, parentImages, pin, navigate, refreshParentImages, onChildUpdated, onChildDeleted }) {
  const [editingChildId, setEditingChildId] = useState(null);

  return (
    <div style={{ display: 'grid', gap: '1rem' }}>
      {children.map(child => (
        <div key={child.id} id={`child-${child.id}`} style={{ background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-md)', padding: '1rem 1.1rem', display: 'grid', gap: '0.9rem' }}>
          {/* Child header */}
...
function CollapsibleSection({ title, titleAs: Tag = 'h2', open, onToggle, children, titleStyle, headerRight }) {
  return (
    <>
...
export default function ParentDashboard() {
  ...
  useEffect(() => {
    const newChildId = location.state?.scrollToChild;
    if (!newChildId || loading) return;
    setGestionOpen(true);
    const el = document.getElementById(`child-${newChildId}`);
    if (el) {
      el.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
    // Clear state so a page refresh doesn't re-scroll
    window.history.replaceState({}, '');
  }, [loading, location.state?.scrollToChild]);
        ...
        <section style={sectionStyle}>
          <CollapsibleSection title="Suivi des enfants" open={suiviOpen} onToggle={() => setSuiviOpen(o => !o)}>
            {loading ? (
              ...
              const effectiveId = selectedSuiviChildId && children.some(c => c.id === selectedSuiviChildId)
                ? selectedSuiviChildId
                : children[0]?.id;
              const selectedChild = children.find(c => c.id === effectiveId);
              return (
                <div>
                  {/* Child selector tabs */}
                  {children.length > 1 && (
                    <div style={childSelectorStyle}>
                      {children.map(child => {
                        const isActive = child.id === effectiveId;
                        return (
                          <button
                            key={child.id}
                            onClick={() => setSelectedSuiviChildId(child.id)}
                            style={{
                              ...childSelectorTabStyle,
                              background: isActive
...
const childSelectorTabStyle = {
  flex: 1, padding: '0.55rem 0.4rem', borderRadius: 10,
