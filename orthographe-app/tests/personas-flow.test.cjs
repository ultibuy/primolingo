...
const DEBUG_UID = 'puppeteer-test-uid';
...
async function run() {
  ...
  await page.evaluateOnNewDocument((progressKey, progress) => {
    localStorage.setItem('ortho_debug', '1');
    localStorage.setItem('debug_uid', 'puppeteer-test-uid');
    localStorage.setItem('debug_child_name', 'Puppeteer');
    localStorage.setItem(progressKey, JSON.stringify(progress));
  }, DEBUG_PROGRESS_KEY, debugProgress);

  await page.goto(`${BASE_URL}/play/${CHILD_ID}`, { waitUntil: 'networkidle2', timeout: 30000 });
  await waitForText(page, 'Puppeteer');
  await page.screenshot({ path: '/tmp/puppeteer-01-dashboard.png', fullPage: true });
  ...
  await page.screenshot({ path: '/tmp/puppeteer-02-shop-popup.png', fullPage: true });
  ...
  await page.screenshot({ path: '/tmp/puppeteer-03-quiz.png', fullPage: true });
